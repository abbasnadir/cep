...
function ensureSuccess(data, error, message) {
    if (error) {
        throw new Error(message ?? error.message);
    }
    return data;
}
function mapArea(area, fallbackLabel) {
    if (!area) {
        return {
            id: "global",
...
function groupByPostId(rows, key = "post_id") {
...
function countByPostId(rows, key = "post_id") {
    return rows.reduce((accumulator, row) => {
        const id = row[key];
        if (!id)
            return accumulator;
...
async function fetchAreasByIds(areaIds) {
    ...
    const { data, error } = await supabase
        ...
        .in("id", areaIds);
    return new Map(ensureSuccess(data ?? [], error, "Failed to load areas").map((row) => [row.id, row]));
}
async function fetchCategoriesByIds(categoryIds) {
    ...
    const { data, error } = await supabase
        ...
        .in("id", categoryIds);
    return new Map(ensureSuccess(data ?? [], error, "Failed to load categories").map((row) => [row.id, row]));
}
async function fetchOrganizationsByIds(organizationIds) {
    ...
    return new Map(ensureSuccess(data ?? [], error, "Failed to load organizations").map((row) => [row.id, row]));
}
export async function fetchProfileById(userId) {
    const { data, error } = await supabase
        .from("profiles")
        .select("id, organization_id, home_area_id, role, public_alias, anonymous_by_default, preferred_language, onboarding_complete, deleted_at")
        .eq("id", userId)
        .is("deleted_at", null)
        .maybeSingle();
...
export async function requireInstitutionProfile(userId) {
...
export function mapProfileResponse(profile, user) {
...
async function fetchPostSupportData(postIds) {
    const [mediaResult, aiResult, commentResult, raiseResult, reportResult] = await Promise.all([
        ...
        supabase
            ...
            .select("post_id, enrichment_status, severity_level, complexity_level, summary, translated_text, hazard_tags, confidence_score, model_version, processed_at")
            .in("post_id", postIds),
        supabase
            .from("post_comments")
            .select("post_id")
    ...
    return {
        mediaRows: ensureSuccess(mediaResult.data ?? [], mediaResult.error, "Failed to load post media"),
        aiRows: ensureSuccess(aiResult.data ?? [], aiResult.error, "Failed to load AI assessments"),
        commentRows: ensureSuccess(commentResult.data ?? [], commentResult.error, "Failed to load comments"),
        raiseRows: ensureSuccess(raiseResult.data ?? [], raiseResult.error, "Failed to load raises"),
        reportRows: ensureSuccess(reportResult.data ?? [], reportResult.error, "Failed to load reports"),
    };
}
function mapAiAssessment(row, fallbackStatus) {
    return {
...
export async function hydratePosts(rows, options) {
    if (!rows.length)
        return [];
    ...
    return rows.map((row) => {
        ...
        return {
            ...
            rankingReason: {
                ...
                engagementWeight: ((raiseCounts[row.id] ?? 0) + (commentCounts[row.id] ?? 0)) / 100,
            ...
            ...(options?.includeExactLocation
                ? {
                    exactLocation: row.latitude != null && row.longitude != null
                        ? {
...
export async function fetchPostDetail(postId, options) {
    const { data, error } = await supabase
        .from("posts")
        .select("id, author_id, category_id, area_id, public_alias_snapshot, description, source_language, display_language, workflow_status, enrichment_status, priority_score, is_anonymous, sanitized_area_label, latitude, longitude, created_at, updated_at")
        .eq("id", postId)
        .maybeSingle();
...
export async function fetchComments(postId, page, limit) {
...
export async function fetchFeed(request) {
    const { page, limit } = parsePagination(request);
    const areaId = typeof request.query.areaId === "string" ? request.query.areaId : undefined;
    const categoryId = typeof request.query.categoryId === "string" ? request.query.categoryId : undefined;
    const status = typeof request.query.status === "string" ? request.query.status : undefined;
    const sort = typeof request.query.sort === "string" ? request.query.sort : "ranked";
    const query = supabase
        .from("posts")
        .select("id, author_id, category_id, area_id, public_alias_snapshot, description, source_language, display_language, workflow_status, enrichment_status, priority_score, is_anonymous, sanitized_area_label, created_at, updated_at")
        .order("created_at", { ascending: false })
        .range(0, 99);
    if (areaId)
        query.eq("area_id", assertUuid(areaId, "areaId"));
    if (categoryId)
        query.eq("category_id", assertUuid(categoryId, "categoryId"));
    if (status)
        query.eq("workflow_status", status);
    const { data, error } = await query;
    const posts = await hydratePosts(ensureSuccess(data ?? [], error, "Failed to load feed posts"));
...
export async function fetchSummary(request, areaId) {
    ...
    const bySeverity = Array.from(posts.reduce((accumulator, post) => {
        const severity = severityByPost.get(post.id) ?? "unknown";
        accumulator.set(severity, (accumulator.get(severity) ?? 0) + 1);
        return accumulator;
    }, new Map())).map(([severity, count]) => ({ severity, count }));
    const byCategory = Array.from(posts.reduce((accumulator, post) => {
        accumulator.set(post.category_id, (accumulator.get(post.category_id) ?? 0) + 1);
        return accumulator;
    }, new Map())).map(([categoryId, count]) => ({
        categoryId,
        label: categories.get(categoryId)?.display_name ?? "Uncategorized",
        count,
    }));
    const byStatus = Array.from(posts.reduce((accumulator, post) => {
        accumulator.set(post.workflow_status ?? "open", (accumulator.get(post.workflow_status ?? "open") ?? 0) + 1);
        return accumulator;
    ...
    const topRows = [...posts]
        ...
        .slice(0, 5);
    const topIssues = await hydratePosts(topRows.map((row) => ({
        ...row,
...
export async function fetchAreas(request) {
    const q = typeof request.query.q === "string" ? request.query.q : undefined;
    const parentAreaId = typeof request.query.parentAreaId === "string"
    ...
    if (type)
        query.eq("area_type", type);
    const { data, error } = await query;
    return ensureSuccess(data ?? [], error, "Failed to load areas").map((row) => mapArea(row));
}
...
export async function createPost(userId, body) {
    ...
    const profile = await requireProfile(userId);
    const areaId = body.location?.mode === "manual" && typeof body.location.areaId === "string"
    ...
    const { data, error } = await supabase
        ...
        display_language: Array.isArray(body.translateTargets) && body.translateTargets[0]
    ...
    if (media.length) {
        const { error: mediaError } = await supabase.from("post_media").insert(media.map((item) => ({
...
export async function toggleRaise(userId, postId) {
    await fetchPostDetail(postId);
    const existingRaiseResult = await supabase
        ...
        .eq("user_id", userId)
        .maybeSingle();
    const existingRaise = ensureSuccess(existingRaiseResult.data, existingRaiseResult.error, "Failed to check raise state");
    let raised = false;
    if (existingRaise?.id) {
...
export async function createReport(userId, postId, body) {
...
export async function fetchInstitutionPostDetail(postId) {
    ...
    const caseViewResult = await supabase
        ...
        .limit(1)
        .maybeSingle();
    const reportRows = ensureSuccess(reportRowsResult.data ?? [], reportRowsResult.error, "Failed to load moderation state");
    const caseView = ensureSuccess(caseViewResult.data, caseViewResult.error, "Failed to load institution case view");
    const organizationMap = await fetchOrganizationsByIds(caseView?.organization_id ? [caseView.organization_id] : []);
    return {
        ...post,
        moderationState: {
...
//# sourceMappingURL=civicData.js.map