...
export async function routesHandler() {
    ...
    try {
        ...
        for (const dir of directory) {
            ...
            if (!/\.(ts|js|cjs|mjs)$/i.test(dir.name))
                continue;
            const module = await import(path.join(parentDir, dir.name));
            const routerObj = module.default;
            for (const obj of routerObj.functions) {
                const fullPath = routerObj.path + (obj.props ?? "");
                router[obj.method](fullPath, authHandler(obj.authorization), rateLimiter.limit(obj.rateLimit, obj.keyType), tryCatch(obj.handler));
            }
        }
        return router;
    }
